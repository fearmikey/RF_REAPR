...
import com.example.rf_reapr.ui.wifi.WifiFingerprintViewModel.FrequencyRange
...
fun WifiChannelGraph(
    ...
    val (minFreq, maxFreq) = when (range) {
        FrequencyRange.FREQ_2_4GHZ -> 2400f to 2495f
        FrequencyRange.FREQ_5GHZ -> 5150f to 5850f
        FrequencyRange.FREQ_6GHZ -> 5925f to 7125f
    }
        ...
        Canvas(modifier = Modifier
            ...
            when (range) {
                ...
                FrequencyRange.FREQ_6GHZ -> {
                    listOf(1, 33, 65, 97, 129, 161, 193).forEach { ch ->
                        val f = 5945f + (ch - 1) * 5f
                        val x = freqToX(f)
                        if (x in 0f..width) {
                            drawContext.canvas.nativeCanvas.drawText("$ch", x, height - 5f, channelPaint)
